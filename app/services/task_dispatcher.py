...
class AgentLoadTracker:
    ...
    def record_task_completion(
        ...
        agent_data["success_rate"] = agent_data["completed_tasks"] / total_tasks if total_tasks > 0 else 1.0
        ...
        if resource_usage:
            for resource, usage in resource_usage.items():
                ...
                agent_data["resource_utilization"][resource] = current * 0.8 + usage * 0.2
    ...
    def get_agent_load_score(self, agent_id: str) -> float:
        """Calculate normalized load score for agent"""
        agent_data = self._agent_loads[agent_id]
        
        active_load = min(agent_data["active_tasks"] / 10.0, 1.0)
        failure_penalty = 1.0 - agent_data["success_rate"]
        time_penalty = min(agent_data["average_execution_time"] / 300.0, 1.0)
    ...
    def get_best_agents(
        ...
        for agent_id, data in self._agent_loads.items():
            ...
            if required_capabilities:
                ...
                capability_match = matches / len(required_capabilities) if required_capabilities else 1.0
            
            load_score = self.get_agent_load_score(agent_id)
            final_score = capability_match * (1.0 - load_score)
...
class TaskQueue:
    ...
    def __init__(self):
        ...
        self._task_dependencies = defaultdict(set)
        self._pending_tasks = {}
...
class TaskDistributionEngine:
    ...
    async def submit_task(
        ...
        try:
            ...
            self.logger.error("Failed to submit task", error=str(e), task_type=task_type)
            raise HTTPException(status_code=400, detail=f"Task submission failed: {str(e)}")
    ...
    async def handle_task_result(self, result: TaskResult) -> None:
        ...
        try:
            ...
            self.logger.error("Failed to handle task result", task_id=result.task_id, error=str(e))
    ...
    async def _select_agent(self, task: AgentTask, candidates: List[str]) -> str:
        ...
        if strategy == LoadBalancingStrategy.LEAST_LOADED:
            return min(candidates, key=lambda agent: self._load_tracker.get_agent_load_score(agent))
    ...
    async def _assign_task_to_agent(self, task: AgentTask, agent_id: str) -> None:
        ...
        try:
            assignment = TaskAssignment(
                ...
                timeout_at=task.metadata.timeout_at
            )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                for task_id, assignment in self._active_assignments.items():
                    if assignment.timeout_at and current_time > assignment.timeout_at:
    ...
    async def _handle_task_timeout(self, task_id: str) -> None:
        ...
        try:
            ...
            await self.message_broker.send_direct_message(
                ...
                payload={"action": "task_timeout", "task_id": task_id}
            )
            
            await self.handle_task_result(result)
            
            self.logger.warning("Task timed out", task_id=task_id, agent_id=assignment.agent_id)
            
        except Exception as e:
            self.logger.error("Failed to handle task timeout", task_id=task_id, error=str(e))
    ...
    async def _process_learning_feedback(self, result: TaskResult) -> None:
        ...
        try:
            if not result.learning_feedback:
                return
            # Process learning feedback for optimization
        except Exception as e:
...
TaskDispatcher = TaskDistributionEngine