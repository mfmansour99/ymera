...
class MessageQueue:
    ...
    async def put(self, message: InterAgentMessage) -> bool:
...
class MessageBroker:
    ...
    def process_message(self, message: dict) -> dict:
        ...
        This is a synchronous wrapper for message processing that validates
        the message format and returns processing status.
        ...
        try:
            ...
            valid_types = ['request', 'response', 'notification', 'broadcast', 'direct']
            ...
            elif message_type in ['broadcast', 'direct']:
                return {
                    'status': 'queued',
                    'message_id': message.get('message_id', str(uuid.uuid4())),
                    'data': message.get('payload'),
            ...
            self.logger.error(f"Message validation error: {ve}")
            return {
                'status': 'error',
                'error': str(ve),
    ...
    async def _queue_for_recipients(
        ...
        for recipient_id in recipients:
            ...
            recipient_message = InterAgentMessage(
                metadata=MessageMetadata(
                    **{k: v for k, v in message.metadata.__dict__.items()},
                    recipient_id=recipient_id
                ),
            ...
            try:
                ...
                if self._redis:
                    await self._redis.hset(
                        ...
                        mapping={k: str(v) for k, v in self._metrics.items()}
                    )
