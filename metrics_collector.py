...
✅ FIXED: Complete implementation with all missing functions
    ...
    def get_statistics(self, metric_key: str) -> Dict[str, float]:
        ...
        with self.lock:
            ...
            try:
                ...
                logger.error(f"Error calculating statistics for {metric_key}: {e}")
                return {}
...
class MetricsCollector:
    ...
    async def _setup_redis_structures(self):
        ...
        try:
            ...
            self.logger.error(f"Error setting up Redis structures: {e}")
    ...
    def _get_system_resource_metrics(self) -> SystemResourceMetrics:
        """
        ✅ FIXED: Complete implementation of system resource metrics collection
        This function was truncated in the original file
        """
        try:
            # CPU metrics
            ...
            try:
                ...
                load_average = list(load_avg_tuple)
    ...
    async def _collect_agent_metrics(self):
        ...
        try:
            ...
            for agent_id, agent_instance in self.agents_registry.items():
                try:
                    # Get agent metrics if available
                    if hasattr(agent_instance, 'get_metrics'):
                        agent_data = await agent_instance.get_metrics()
                    else:
                        # Default metrics
                        agent_data = {
                            'agent_id': agent_id,
                    ...
                    metrics = [
                        MetricPoint(
                            f"agent_{agent_id}_task_count",
                            agent_data.get('task_count', 0),
                            timestamp,
                            MetricType.COUNTER,
                            MetricScope.AGENT,
                            {"agent_id": agent_id, "agent_type": agent_data.get('type', 'unknown')}
                        )
                    ]
                    ...
                    self.logger.warning(f"Error collecting metrics for agent {agent_id}: {e}")
                    
        except Exception as e:
            self.logger.error(f"Error in agent metrics collection: {e}")
    
    async def _collect_learning_metrics(self):
        """Collect learning engine metrics"""
        try:
            # Placeholder for learning metrics collection
            timestamp = datetime.utcnow()
        ...
        async with self.buffer_lock:
            ...
            try:
                ...
                for metric in self.metrics_buffer:
                    ...
                    pipeline.lpush(key, json.dumps(metric.to_dict()))
                    pipeline.ltrim(key, 0, 999)  # Keep last 1000 entries
            ...
            for key in keys:
                try:
                    ...
                    if valid_entries:
                        ...
                        if valid_entries:
                            await self.redis_client.rpush(key, *valid_entries)
                    else:
                        await self.redis_client.delete(key)
    ...
    async def get_metrics(
        ...
        try:
            ...
            # Build pattern
            if scope and metric_name:
                pattern = f"{self.redis_keys['metrics']}:{scope.value}:{metric_name}"
            elif scope:
    ...
    async def get_aggregated_metrics(
        ...
        return self.aggregator.get_statistics(metric_key)
    
    def register_agent(self, agent_id: str, agent_instance: Any):
        """Register an agent for metrics tracking"""
        self.agents_registry[agent_id] = agent_instance
        self.logger.info(f"Agent registered for metrics tracking: {agent_id}")
    
    def unregister_agent(self, agent_id: str):
        """Unregister an agent from metrics tracking"""
        if agent_id in self.agents_registry:
            del self.agents_registry[agent_id]
            self.logger.info(f"Agent unregistered from metrics tracking: {agent_id}")
